...
namespace TikkieAPIConsoleTests
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                //var result = await client.CreatePlatformAsync(new TikkieAPI.Models.PlatformRequest
                //{
                //    Name = "Test platform",
                //    Email = "client@platform.com",
                //    PhoneNumber = "0617386240",
                //    Usage = TikkieAPI.Enums.PlatformUsage.PaymentRequestForMyself
                //});
                Console.WriteLine("Platforms:");
                Console.WriteLine(JsonConvert.SerializeObject(platforms, Formatting.Indented));

                //var user = await client.CreateUserAsync(platforms.First().PlatformToken, new TikkieAPI.Models.UserRequest
                //{
                //    Name = "Chus Second",
                //    PhoneNumber = "0617386240",
                ...
                //var paymentCreationResult = await client.CreatePaymentRequestAsync(new TikkieAPI.Models.PaymentRequest
                //{
                ...
                var userPaymentRequests = await client.GetUserPaymentRequestsAsync(new TikkieAPI.Models.UserPaymentRequest
                {
                    PlatformToken = platforms.First().PlatformToken,
                    UserToken = users.First().UserToken,
                    Limit = 100
                ...
                var singlePaymentRequest = await client.GetPaymentRequestAsync(new TikkieAPI.Models.SinglePaymentRequest
                {
                    PlatformToken = platforms.First().PlatformToken,
                    UserToken = users.First().UserToken,
